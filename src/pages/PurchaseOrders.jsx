...
export default function PurchaseOrders() {
  ...
  const updatePOItem = (index, field, value) => {
    setNewPO(prevPO => {
      ...
      const credit = parseFloat(prevPO.credit) || 0; const discount = parseFloat(prevPO.discount) || 0;
      return { ...prevPO, items: updatedItems, estSubtotal, subtotal: shipSubtotal, estTotal: estSubtotal + tax + shipping - credit - discount, total: shipSubtotal + tax + shipping - credit - discount };
    });
  };
  ...
  const openEditOrder = (order) => {
    ...
    const credit = parseFloat(order.credit) || 0; const discount = parseFloat(order.discount) || 0;
    // Format invoiceDate for input field
    let invoiceDateStr = '';
    if (order.invoiceDate) {
      const d = order.invoiceDate.toDate ? order.invoiceDate.toDate() : new Date(order.invoiceDate);
      invoiceDateStr = d.toISOString().split('T')[0];
    }
  ...
  const savePackOrder = async () => {
    ...
    const credit = parseFloat(packingOrder.credit) || 0;
    const discount = parseFloat(packingOrder.discount) || 0;
    ...
    const updateData = cleanForFirestore({
      ...
      total: newSubtotal + tax + shipping - credit - discount
    });
  ...
  // The document itself lives in functions/orderDocument.mjs so the emailed
  // copy and this printed copy can never drift apart. This just opens it.
  const printPO = (order, printType = 'invoice') => {
    const printContent = renderOrderDocument(order, printType, {
      items,
      organization,
      branding: (org, opts) => DB.brandingHtml(org, opts)
    });
    const printWindow = window.open('', '_blank');
    printWindow.document.write(printContent); printWindow.document.close(); printWindow.print();
  };
        ...
        <div className="modal-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) { setShowCreate(false); setEditMode(false); resetForm(); } }}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 900, maxHeight: '90vh', overflow: 'auto' }}>
            ...
            <div className="modal-body" style={{ padding: 20 }}>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                {newPO.useShipTo && (
                  <div style={{ marginTop: 10 }}>
                    <input
                      type="text"
                      placeholder="Ship to company / recipient name (e.g. The Camouflage Shop)"
                      value={newPO.shipToCompany || ''}
                      onChange={e => setNewPO({ ...newPO, shipToCompany: e.target.value })}
                      style={{ width: '100%', padding: 10, borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-input)', color: 'var(--text-primary)', marginBottom: 8, boxSizing: 'border-box' }}
                    />
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8, fontStyle: 'italic' }}>
                      For drop-shipping — this name prints on the shipping label instead of the billing customer's name. Leave blank to use the billing customer name.
                    </div>
                    <textarea 
                ...
                <div style={{ marginBottom: 20, overflowX: 'auto', overflowY: 'auto', maxHeight: '50vh' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 15 }}>
                    <div style={{ width: 220, background: 'var(--bg-badge-green)', padding: 15, borderRadius: 8, border: '2px solid #388e3c' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', alignItems: 'center', color: '#2e7d32' }}><span>Credit:</span>
                        <input type="number" value={newPO.credit === '' || newPO.credit === undefined || newPO.credit === null ? '' : newPO.credit} onChange={e => updateTaxShipping('credit', e.target.value)} style={{ width: 70, padding: 5, textAlign: 'right', color: '#2e7d32' }} step="0.01" min="0" placeholder="0.00" /></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', alignItems: 'center', color: '#2e7d32' }}><span>Discount:</span>
                        <input type="number" value={newPO.discount === '' || newPO.discount === undefined || newPO.discount === null ? '' : newPO.discount} onChange={e => updateTaxShipping('discount', e.target.value)} style={{ width: 70, padding: 5, textAlign: 'right', color: '#2e7d32' }} step="0.01" min="0" placeholder="0.00" /></div>
              ...
              {/* Terms and Notes */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 2fr', gap: 15, marginBottom: 20 }}>
                <div>
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: 5, fontWeight: 600 }}>Invoice Date</label>
                  <input 
                    type="date" 
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: 5, fontWeight: 600 }}>Customer PO #</label>
                  <input 
                    type="text" 
                    placeholder="Optional — customer's PO number"
                    value={newPO.customerPO || ''} 
                    onChange={e => setNewPO({ ...newPO, customerPO: e.target.value })}
                    style={{ width: '100%', padding: 10, borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg-input)', color: 'var(--text-primary)' }}
                  />
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 3 }}>Reference for customer's records</div>
                </div>
                <div>
      ...
      {selectedOrder && (
        <div className="modal-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) closeOrderModal(); }}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 700, maxHeight: '90vh', overflow: 'auto' }}>
            <div className="modal-header">
              <div><h3 style={{ margin: 0 }}>{selectedOrder.poNumber}</h3><p style={{ color: 'var(--text-muted)', margin: '5px 0 0 0' }}>{selectedOrder.customerName}{selectedOrder.customerPO ? <span style={{ marginLeft: 10, padding: '2px 8px', borderRadius: 4, background: 'rgba(33,150,243,0.12)', color: '#1976d2', fontSize: 12, fontWeight: 600 }}>Customer PO: {selectedOrder.customerPO}</span> : null}</p></div>
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        {shipping <= 0 && newPO.shippingBilledToCustomer && <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', color: 'var(--text-muted)', fontSize: 13 }}><span>Shipping:</span><span>Billed to customer's carrier</span></div>}
